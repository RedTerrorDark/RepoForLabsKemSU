...
string arabicToRoman(int arabic) {
  ...
  for (auto pairRomanNumbers : romanValues) {
    while (arabic >= pairRomanNumbers.first) {
      resultRoman += pairRomanNumbers.second;
      arabic -= pairRomanNumbers.first;
    }
  }
  return resultRoman;
}
int romanToArabic(const string& roman) {
  ...
  int resultArabic = 0, currentRomanValue, nextRomanValue;

  for (size_t index = 0; index < roman.length(); ++index) {
    currentRomanValue = arabicValues[roman[index]];
    if (index + 1 < roman.length()) {
      nextRomanValue = arabicValues[roman[index + 1]];
      if (currentRomanValue < nextRomanValue) {
        resultArabic += nextRomanValue - currentRomanValue;
        ++index;
        continue;
      }
    }  
    resultArabic += currentRomanValue;
  }
  return resultArabic;
}
...
int main() {
  ...
  int option, startArabic, endRoma;
  string startRoman;
  bool success = false;
